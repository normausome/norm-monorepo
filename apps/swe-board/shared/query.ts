...
/** `?k=a,b&k=c` reads as `[a, b, c]`. Unknown values are dropped, repeats kept once. */
function manyOf<T extends string>(values: readonly T[], raws: string[]): T[] {
  const picked = raws.flatMap((raw) => raw.split(",")).map((s) => oneOf(values, s.trim()))
